...
it('creates a new scene', () => {
  const scene = new GameScene('First', bgImage.background[0], 'move', 'Dialog1');
  expect(scene).toBeTruthy();
});

it('creates a new dialog', () => {
  const newDialogue = new Dialogue('Dialog1', new Dialogue('Dialog1', 'Stage 1 Cleared', scenario.dialog[0][0], 'Second'));
...
it('dialogues are an array of arrays', () => {
  expect(Array.isArray(dialog[0])).toBeTruthy();
});
